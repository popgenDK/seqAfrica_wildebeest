...
# all_pops = parallel::mclapply(1:100, FUN=function(x){find_graphs(f2s, verbose=FALSE, max_admix= 2, outpop = 'Okapi', stop_gen = 80, stop_gen2=10, admix_constraints = constraints)}, mc.cores=60)
# winners_all_pops = lapply(all_pops, function(x){x %>% slice_min(score, with_ties = FALSE)})
