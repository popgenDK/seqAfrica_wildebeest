...
splitt <- unlist(sapply(allbed$chr, splitChrs, bed=allbed))
...
for(i in 1:length(w)){
    ...
    start <- badbed[badbed$chr==c & badbed$start < p[length(p)] & badbed$end > p[1],]$start
    end <- badbed[badbed$chr==c  & badbed$start < p[length(p)] & badbed$end > p[1],]$end
    if(length(start) > 0){
